...
const RSS_SOURCES = {
  ...
  // YouTube каналы по вашей тематике
  // Замените эти примеры на реальные каналы про GetCourse, Prodamus, веб-разработку
...
async function dailyDigest() {
  ...
  try {
    ...
    const digestPrompt = `Ты — эксперт по автоматизации онлайн-образования, веб-разработке и платёжным системам.

Из этого списка материалов выбери ТОЛЬКО те, которые релевантны для следующих тем:
- Автоматизация GetCourse (интеграции, настройка, кастомизация)
- Prodamus.XL (настройка, интеграция, оплаты)
...
📊 **ДАЙДЖЕСТ ДНЯ: GetCourse, Продажи и Автоматизация**
...
Используй эмодзи, структурируй текст, пиши конкретно для практиков.`;
    ...
    // Сохраняем ТОП-3 в Google Sheets
    const topArticles = allArticles.slice(0, 3);
    
    for (const article of topArticles) {
      try {
        const analysisPrompt = `Проанализируй этот материал для базы знаний по GetCourse и автоматизации:
...
{
  "keywords": "5-7 ключевых слов через запятую (GetCourse, Prodamus, лендинг, скрипт и т.д.)",
  "category": "одна категория: GetCourse/Prodamus/Лендинги/Скрипты/Кастомизация/Маркетинг",
  "analysis": "краткое резюме в 1-2 предложениях с практической пользой",
  "idea": "конкретная идея применения для GetCourse, Prodamus.XL или создания продающего сайта"
}`;
        ...
        try {
          ...
          analysisData = {
            keywords: 'автоматизация, онлайн-школа',
            category: 'Общее',
            analysis: analysisResult.response.text().substring(0, 200),
            idea: 'Применить в своем проекте'
          };
        }
        ...
        console.log(`✅ Сохранено в таблицу: ${article.title.substring(0, 50)}...`);
        
        // Пауза между запросами к Gemini (избегаем rate limit)
        await new Promise(resolve => setTimeout(resolve, 2000));
        
      } catch (error) {
        console.log(`⚠️ Ошибка сохранения статьи: ${error.message}`);
      }
    }
    ...
    try {
      await bot.sendMessage(CHANNEL_ID, 
        `❌ Ошибка при создании дайджеста: ${error.message}`
      );
    } catch (e) {
...
- Prodamus.XL (интеграции, скрипты, кастомизация, настройка платежей, интеграции)
- Оформление личных кабинетов (дизайн, UX/UI, верстка)
- Создание продающих сайтов и лендингов
...
Целевая аудитория:
- Владельцы онлайн-школ на GetCourse, Prodamus.XL
...
**1. Заголовок** (цепляющий, с цифрами или вопросом)
   Примеры: "7 скриптов для GetCourse, которые увеличат продажи на 30%"
           "Как кастомизировать личный кабинет GetCourse или Prodamus.XL без программиста"
...
bot.onText(/\/analyze (.+)/, async (msg, match) => {
  ...
  try {
    ...
    const prompt = `Проанализируй этот материал как эксперт по GetCourse, Prodamus.XL и веб-разработке: ${url}
...
cron.schedule('0 9 * * *', () => {
  ...
  timezone: "Asia/Yakutsk" // Ваш часовой пояс
});
...
console.log('🎯 Тематика: GetCourse, Prodamus, лендинги, автоматизация');
